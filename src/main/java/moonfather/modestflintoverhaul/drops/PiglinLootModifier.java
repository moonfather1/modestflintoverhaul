...
{
    protected PiglinLootModifier(LootItemCondition[] conditionsIn, int priority)
    {
    ...
    {
        if (context.getQueriedLootTableId().equals(BuiltInLootTables.PIGLIN_BARTERING.identifier()))
        {
            int count = 0;
            ...
            if (count > 0)
            {
                generatedLoot.add(new ItemStack(ItemsAndBlocks.ItemGravelUnsearched.get(), count));
            }
        }
    ...
    @Override
    public MapCodec<? extends IGlobalLootModifier> codec() { return CODEC.get(); }
